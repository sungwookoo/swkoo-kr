...
const sections = [
  ...
  {
    n: 3,
    title: '변경 내용을 확인하고 배포합니다',
    body: '대상 브랜치와 파일별 현재·제안 내용을 먼저 확인하고 동의합니다. 현재 기본 브랜치 main만 지원합니다:',
    bullets: [
      'Dockerfile·.github/workflows/build.yml이 없으면 동의 후 main에 직접 생성합니다. 템플릿과 동일한 기존 파일은 유지합니다',
      '기존 파일이 다르면 자동 덮어쓰기 없이 사용자 요청으로 초안 PR을 만듭니다. 차이가 있다고 잘못된 설정으로 판정하는 것은 아닙니다. 변경을 검토·병합한 뒤 다시 조회하고 배포를 계속하세요',
      'PR 생성만으로 신규 배포를 등록하지 않습니다. 이미 배포 중인 앱은 병합 시 기존 자동 배포가 실행될 수 있습니다',
      ...
      '등록 결과에서 소스 커밋과 배포 진행상황을 확인하세요. 중간 실패 시 완료된 단계와 소스 링크를 안내하며 이미 생성된 파일을 자동 삭제하지 않습니다',
    ],
  },
  {
    n: 4,
  ...
  {
    n: 5,
    title: 'npm 의존성 취약점 수정은 직접 요청하고 PR로 검토합니다',
    body: '관리 화면의 npm 의존성 취약점 수정 PR에서 근거 링크·해결된 항목·남은 항목과 버전 변경을 확인한 뒤 초안 PR을 요청할 수 있습니다. 요청 시점의 npm 검사이며 AI 코드 분석·정기 재검사·알림은 제공하지 않습니다. 앱 테스트·빌드·DB 연결은 별도 검증이 필요하고, 자동 병합하지 않습니다.',
    bullets: [
